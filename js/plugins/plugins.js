 ...
 * Version 1.1
 */ (function ($) {
    $.fn.staticTooltip = function (options) {
        var defaults;
        defaults = $.extend({
            position: "left",
            content: '',
        ...
        this.each(function () {
            ...
            $(this).hover(function () {
                $('body').append('<div class="staticTooltip ' + defaults.position +  '"><div class="tip"></div><div class="staticTooltipContent">' + defaults.content + '</div></div>');
                leftPos = defaults.position === 'left' ? $(this).offset().left - ($('.staticTooltip').width() + defaults.padding) : leftPos;
                leftPos = defaults.position === 'top' || defaults.position === 'bottom' ? $(this).offset().left - (($('.staticTooltip').width() / 2) - ($(this).width() / 2)) -  defaults.padding: leftPos;
                leftPos = defaults.position === 'right' ? $(this).offset().left + ($(this).width() + defaults.padding) : leftPos;

                topPos = defaults.position === 'left' || defaults.position === 'right' ? $(this).offset().top + ( ($(this).height() / 2) - ($('.staticTooltip').height() / 2)) - defaults.padding : topPos;
                topPos = defaults.position === 'top' ? $(this).offset().top - ($('.staticTooltip').height() + $('.staticTooltip .tip').height() + defaults.padding) : topPos;
                topPos = defaults.position === 'bottom' ? $(this).offset().top + $(this).height() + defaults.padding : topPos;
...
(function ($) {
    $.fn.youtubeSelector = function (options) {
        var defaults, opts, output, app_utils;
        app_utils = {
        ...
        this.each(function () {
            var selector, events, loadBar, recycleIcon, typeTimer, doneTyping, afterLoading, _this = $(this), droppableDropped;

            $('<div class="youtubeVideoSelector wrapper"><div class="youtubeVideoSearcher"><input type="text" value="' + opts.label + '" name="youtubeSelector" /></div><div class="yourVideoSelectorResults"></div><div class="youtubeVideoDropBox"><div class="dropBox"><h3>' + opts.dropBoxLabel + '</h3></div></div></div>').insertBefore(this);
            ...
            events = (function () {
                ...
                selector.find('.youtubeCancelBtn').click(function () {
                    ...
                    if (selector.find('.yourVideoSelectorResults ul').length) {
                        selector.find('.yourVideoSelectorResults ul').html('<li>{message}</li>'.replace('{message}', opts.noResults));
                    }
                    return false;
                });
            ...
            droppableDropped = function (event, ui) {
                var draggable, vidURL = $(ui.draggable).find('img').attr('alt').split('?')[0];
                        ...
                        if (opts.allowed) {
                            if (selector.find('.dropBox div.videos').length < opts.allowed) {
                        ...
                        selector.find('.dropBox a').click(function () {
                            ...
                            if (!selector.find('.dropBox').html().length) {
                                selector.find('.dropBox').html('<h3>' + opts.dropBoxLabel + '</h3>');
                            }
                            return false;
                        });
            };
            ...
            afterLoading = function () {
                ...
                selector.find('.dropBox').droppable({
                    ...
                    drop: droppableDropped
                });
            };

            doneTyping = function (query) {

                $.getJSON("http://gdata.youtube.com/feeds/api/videos/?q=" + query + "&max-results=" + opts.results + "&v=2&alt=json-in-script&callback=?", function (data) {
            ...
            if (app_utils.formString(_this.html()).length > 0) {
                droppableDropped(null, {
                    draggable: $('<li class="ui-draggable"><img src="http://img.youtube.com/vi/' +  app_utils.formString(_this.html()) + '/1.jpg" alt="/' + app_utils.formString(_this.html()) + '?"/></li>')
                });
            }
